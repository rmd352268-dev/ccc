...
<head>
    ...
    <title>Secondary Enclave Clearance (Step 2/3) // Payate CC</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ambient-overlay {
            ...
            background: 
                radial-gradient(circle at 50% 20%, rgba(0, 229, 255, 0.12) 0%, transparent 55%),
        ...
        .admin-auth-card {
            ...
            box-shadow: 0 0 60px rgba(0, 229, 255, 0.2), 0 30px 60px -12px rgba(0, 0, 0, 0.95);
            backdrop-filter: blur(24px);
        }

        .hud-corner {
            position: absolute; width: 14px; height: 14px; border-color: var(--neon-cyan);
        ...
        .auth-badge {
            ...
            font-size: 11px; font-weight: 800; color: var(--neon-cyan); margin-bottom: 10px;
            letter-spacing: 1.5px; text-transform: uppercase;
        }
        .auth-title {
            font-size: 24px; font-weight: 900; color: #ffffff; letter-spacing: 1px;
            text-transform: uppercase; text-shadow: 0 0 15px rgba(0, 229, 255, 0.4);
        }
        .auth-subtitle {
        ...
        .form-label-tag { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: var(--neon-cyan); }
        ...
        .form-control:focus {
            border-color: var(--neon-cyan); background: rgba(7, 12, 22, 0.98);
        ...
        .btn-proceed {
            ...
            box-shadow: 0 0 30px rgba(0, 229, 255, 0.45); transition: all 0.2s ease;
        }
        .btn-proceed:hover {
            background: linear-gradient(135deg, #38bdf8 0%, #00e5ff 100%);
            transform: translateY(-2px); box-shadow: 0 0 40px rgba(0, 229, 255, 0.75);
        }
    </style>
</head>
<body>
    ...
    <div class="admin-auth-card">
        ...
        <div class="step-progress-bar">
            ...
            <div class="step-pill active" title="Step 2: Secondary Key (Active)"></div>
            <div class="step-pill" title="Step 3: Master PIN"></div>
        </div>

        <div class="auth-header">
            <div class="auth-badge">
                <i class="fa-solid fa-key"></i> STAGE 2/3 // SECONDARY ENCLAVE KEY
            </div>
            <h1 class="auth-title">PAYATE CC ADMIN</h1>
            <p class="auth-subtitle">Level-2 Enclave Decryption Barrier</p>
        </div>
        ...
        <form action="{{ route('admin.login.step2.post') }}" method="POST">
            @csrf

            <!-- Secondary Security Key -->
            <div class="form-group">
                <label class="form-label" for="secondary_key">
                    <span>Secondary Enclave Key (Level 2)</span>
                    <span class="form-label-tag">*** SEC_KEY_2</span>
                </label>
                <div class="input-wrap">
                    <i class="fa-solid fa-fingerprint input-icon-left"></i>
                    <input type="password" name="secondary_key" id="secondary_key" class="form-control" placeholder="Enter Secondary Enclave Key" required autofocus autocomplete="off">
                </div>
            </div>

            <button type="submit" class="btn-proceed">
                <span>VERIFY LEVEL 2 & PROCEED</span> <i class="fa-solid fa-arrow-right"></i>
            </button>
        </form>
    </div>
    ...
    <script>
        ...
        function drawMatrix() {
            ...
            for (let i = 0; i < drops.length; i++) {
                ...
                ctx.fillStyle = Math.random() > 0.9 ? '#ffffff' : '#00e5ff';
